...
{
    //[TestFixture]
    public class TestTemplateRole : TestGeneric
    {
        long _CountOfDbo;
        long _CountOfIdx;
        ...
        IEnumerable<idaxTemplateRole> dbo;
        IEnumerable<IDAX_TEMPLATE_ROLE> idx;
        ...
        public new void SetUp()
        {
            ...
            dbo = RegisterService.templateRoleRepo.GetAll_dbo().Where(w => scenarioId.Equals(w.ScenarioId.ToString())).ToList();
            idx = RegisterService.templateRoleRepo.GetAll_idaxExport().Where(w => scenarioId.Equals(w.ScenarioId)).ToList();
                ...
                if (dbo != null && idx != null)
                {
                    ...
                    foreach (var _idx in idx)
                    {
                        var _dbo = dbo.Where(w => 
                                        _idx.Role.Equals(w.RoleDescription)
